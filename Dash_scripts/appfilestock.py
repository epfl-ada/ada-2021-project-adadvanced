...
def update_action_dashboard(action):
      ...
      fig.add_trace(go.Scatter(x=dash_datas_action.index,y=dash_datas_action.loc[:,action],
                              mode="lines",name="Derivative [" + action + "]"),row=2,col=1)
      ...
      fig.update_layout(title = {'text': "Correlation between brexit agitation and '" + dash_pearson.loc[action,"Company Name"].lower() + "' action",
                        'y':0.02,
                        'x':0.5,
                        'xanchor': 'center',
